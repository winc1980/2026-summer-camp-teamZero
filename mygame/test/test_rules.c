 ...
 * test_rules.c
 * --------------------------------------------------------------------------
 * DSの画面を起動せず、macOS/Linuxの普通のCコンパイラでゲームルールだけを
 * 自動確認する。描画やキー入力ハードウェアからgame/boardを分離したため、
 * 同じルールコードをPC上でも呼べる。
 *
 * 事前資料との対応:
 * - 3章「関数まわり」 : テストを目的ごとの関数へ分割
 * - 4章「ポインタ」   : Game *を更新関数へ渡す
 * - 6章「構造体」     : テスト用GameInputを組み立てる
 * - 11章「デバッグ」  : assertで期待値を機械的に確認する
 * - ユニットテストという開発方法自体は、事前資料より実践的な発展内容
 */
