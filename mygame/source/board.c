 ...
 * board.c
 * --------------------------------------------------------------------------
 * 8×6盤面に関する「できる／できない」のルールをまとめる。
 * 描画方法には一切触れないため、画像を差し替えても移動ルールは変わらない。
 *
 * 事前資料との対応:
 * - 3章「関数まわり」 : 小さな判定関数への分割、static関数
 * - 4章「ポインタ」   : const Game *で同じ試合状態を読み取る
 * - 5章「配列と文字列」: 2次元配列、境界チェック
 * - 6章「構造体」     : Game内のUnit配列とterrain配列
 * - 11章「デバッグ」  : 不正な配列番号・盤外座標を最初に拒否する防御
 * - A/B/C固有の移動・攻撃仕様は、このゲーム独自で資料外
 */
