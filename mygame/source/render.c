...
static void fillRect(u16 *graphics, int x, int y, int width, int height,
                     u16 color) {
  int row;
  int column;
...
static void makeUnitGraphics(u16 *graphics, Player owner, UnitType type) {
  const u16 *src = (const u16 *)characterimageBitmap;
  int imgWidth = 32;
  int srcY = (owner * 192) + (type * 64);
  int srcX = 0;
  for (int row = 0; row < TILE_SIZE; row++) {
    for (int column = 0; column < TILE_SIZE; column++) {
      graphics[(row * TILE_SIZE + column)] =
          src[(srcY + row) * imgWidth + (srcX + column)];
    }
  }
}
...
static u16 terrainColor(TerrainType terrain) {
  ...
  case TERRAIN_MOUNTAIN:
    return makeColor(17, 12, 7);
  case TERRAIN_RIVER:
    return makeColor(4, 16, 28);
  case TERRAIN_BUILDING:
    return makeColor(19, 19, 19);
  case TERRAIN_PLAIN:
  default:
    return makeColor(7, 22, 8);
  }
}
 ...
 */
static void setBitmapSprite(int id, int x, int y, int priority, int alpha,
                            u16 *graphics) {
  oamSet(&oamMain, id, x, y, priority, alpha, SpriteSize_32x32,
         SpriteColorFormat_Bmp, graphics, -1, false, false, false, false,
         false);
}
  ...
  if (game->phase == PHASE_SELECT_MOVE) {
    ...
    for (i = 0; i < UNIT_COUNT && count < OAM_HIGHLIGHT_MAX; i++) {
      if (boardCanAttack(game, game->selectedUnit, i)) {
        setBitmapSprite(OAM_ATTACK_BASE + count, game->units[i].x * TILE_SIZE,
                        game->units[i].y * TILE_SIZE, 2, 8, attackGraphics);
        count++;
      }
    }
  }
...
static void renderStatusScreen(const Game *game) {
   ...
   */
  if (hasLastConsoleGame &&
      memcmp(&lastConsoleGame, game, sizeof(*game)) == 0) {
    return;
  }
  ...
  snprintf(line, sizeof(line), "プレイヤー%d  %s", (int)game->currentPlayer + 1,
  ...
  if (game->phase == PHASE_SELECT_ACTION) {
    ...
    japaneseTextDraw(uiPixels, 12, 40, line,
                     !canAttack                              ? disabled
                     : game->selectedAction == ACTION_ATTACK ? yellow
                                                             : white);
  ...
  for (i = 0; i < TEAM_SIZE; i++) {
    ...
    snprintf(line, sizeof(line), " %c HP:%3d %s", unitTypeLetter(unit->type),
             unit->hp, unitStatus(unit));
...
void renderInit(void) {
  ...
  for (owner = 0; owner < 2; owner++) {
    for (type = 0; type < TEAM_SIZE; type++) {
      unitGraphics[owner][type] =
          oamAllocateGfx(&oamMain, SpriteSize_32x32, SpriteColorFormat_Bmp);
      /* owner/typeはintループ変数なのでenum型へ明示キャストする（2章）。 */
      makeUnitGraphics(unitGraphics[owner][type], (Player)owner,
                       (UnitType)type);
    }
  }
...
void renderVBlank(void) { oamUpdate(&oamMain); }
