 ...
 * render.c
 * --------------------------------------------------------------------------
 * 上画面へ8×6盤面・ユニット・カーソル、下画面へ日本語の状態表示を描く。
 * ルール判定はboard.c/game.cへ任せ、ここでは「どう見せるか」だけを扱う。
 *
 * 事前資料との対応:
 * - 2章「型と数値」       : u16、色成分、キャスト
 * - 4章「ポインタ」       : VRAMを指すu16 *、const Game *
 * - 5章「配列と文字列」   : ピクセル配列、snprintf、memcmp/memcpy
 * - 7章「メモリ」         : VRAM、静的配列、スプライト画像の確保
 * - 8章「ビット演算」     : 文字ビット判定、色データ
 * - 10章「ゲームコードの型」: 状態から毎フレーム表示を作る
 * - 12章「開発環境」      : BlocksDSで動くコード
 * - 背景モード、VRAMバンク、OAM、ARGB16、VBlankは事前資料外。
 *   BlocksDSのbackgrounds/spritesチュートリアルとlibnds APIに基づく。
 */
