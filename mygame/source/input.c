 ...
 * --------------------------------------------------------------------------
 * libndsのkeysDown()が返すビット列を読み、GameInput構造体へ変換する。
 * game.cはKEY_AなどのDS固有定数を直接使わず、「決定」「取消」「上下移動」
 * という意味だけを受け取る。
 *
 * 現在は、両プレイヤーが十字キー・A・B・STARTを共有し、ターンごとに
 * 本体を交代で操作する共通操作方式。将来、左右分割方式にする場合は、
 * 主にこのファイルでGameInputへの割り当てを変更する。
 *
 * 呼び出し順:
 * 1. main.cのscanKeys()がlibnds内部のキー状態を更新す
...
GameInput inputReadShared(void)
{
    ...
    /* ローカル変数inputの全メンバが呼び出し元へ値としてコピー。 */
    return input;
}
