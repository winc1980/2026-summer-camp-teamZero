 ...
 * game.c
 * --------------------------------------------------------------------------
 * キャラ選択→移動→行動→攻撃対象というゲーム進行、ターン交代、HP、勝敗を
 * 管理する。盤面ルールはboard.c、見た目はrender.cへ任せている。
 *
 * 事前資料との対応:
 * - 3章「関数まわり」 : static関数、引数、戻り値による処理分割
 * - 4章「ポインタ」   : Game *を通して1つの状態を更新する
 * - 5章「配列と文字列」: units[]、message[]、安全なvsnprintf
 * - 6章「構造体」     : Game、Unit、GameInput
 * - 10章「ゲームコードの型」: update関数と状態遷移
 * - 11章「デバッグ」  : 不正選択を拒否し、状態を壊さずreturnする
 * - PHASE_*を使った有限状態機械と可変長引数は、具体例としては資料外
 */
